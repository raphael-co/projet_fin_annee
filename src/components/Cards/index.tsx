...
const Cards: FC = () => {
  ...
  const getContractStakeData = useCallback(() => {
    ...
    return {
      ...
      percentage: `${getPercentage(
        formatted.nodes,
        formatted.stake
      )}% ${translate('Of_Total_Stake', localStorage.getItem('langage'))}`
    };
  ...
  const getNodesNumber = useCallback(() => {
    if (!totalNetworkStake.data || !nodesNumber.data) {
      ...
      return {
        ...
        percentage: loading
          ...
          : translate('Data_Unavailable', localStorage.getItem('langage'))
      };
    }
    ...
    return {
      ...
      percentage: `${getPercentage(
        formatted.nodes,
        formatted.stake
      )}% ${translate('Of_Total_Nodes', localStorage.getItem('langage'))}`
    };
  ...
  const getDelegationCap = useCallback(() => {
    ...
    return {
      ...
      percentage: `${getPercentage(
        formatted.stake,
        formatted.value
      )}% ${translate('Filled', localStorage.getItem('langage'))}`
    };
  ...
  const cards: Array<CardType> = [
    {
      label: translate('Contract_Stake', localStorage.getItem('langage')),
      data: getContractStakeData(),
      colors: ['#2044F5', '#1B37C0'],
      icon: <Logo />
    },
    {
      label: translate('Number_of_Users', localStorage.getItem('langage')),
    ...
    {
      label: translate('Number_of_Nodes', localStorage.getItem('langage')),
      icon: <FontAwesomeIcon icon={faServer} />,
      colors: ['#36CA8C', '#2BA572'],
      data: getNodesNumber()
    },
    {
      label: translate('Computed_APR', localStorage.getItem('langage')),
      colors: ['#FBC34C', '#D49712'],
      icon: <FontAwesomeIcon icon={faLeaf} />,
      data: {
        value: getAnnualPercentage(),
        percentage: translate(
          'Computed_APR_Data_Percentage',
          localStorage.getItem('langage')
        )
      }
    },
    {
      label: translate('Service_Fee', localStorage.getItem('langage')),
    ...
    {
      label: translate('Delegation_Cap', localStorage.getItem('langage')),
